    ...
    while($row = $result->fetch_assoc())
      {
        ...
        if($row['tipo']=='P' && strlen($row['valor'])==0 and ($studio=='231' or $studio=='234' or $studio=='275' or $studio=='276' or $studio=='278' or $studio=='720' or $studio=='876' or $studio=='909' or $studio=='721' or $studio=='214' or  $studio=='923' or $studio=='941' or $studio=='947' or $studio=='948' or $studio=='949' or $studio=='950' or $studio=='960' or $studio=='961' or $studio=='1014' or $studio=='1015' or $studio=='1016' or $studio=='1017' or $studio=='1018' or $studio=='1019' or $studio=='1020' or $studio=='1021' or $studio=='1022' or $studio=='1023' or $studio=='1024' or $studio=='1025' )){
            $nle+=1;
            $nle-=1;
        }else{
        $pdf->Cell($row['posini']);
        ...
        if($row['concepto']=='OBSERVACION MICROSCOPICA:' or $row['concepto']=='CITOLOGIA:'){
          ...
          if($studio=='275' or $studio=='961' or $studio=='1016'){
            switch($row['concepto']){
...
los grupos A, C o G; a las 48 horas de incubación.';
                      break;
                    case 'SBHGA':
                      $de='Streptococcus pyogenes (Estreptococo Betahemolítico
del grupo "A").';
                      break;  
                    case 'SBHGC':
                  ...
                  $pdf->MultiCell(120,5,utf8_decode(trim($de)),0,'L');
                  break;
                ...
                case 'RESULTADO FINAL:':
                  ...
                  switch ($row['valor']) {
                    case 'DSBH':
                      $re1='Aproximadamente el 80% de las faringitis son de origen viral
por lo que no se obtiene crecimiento mediante éste estudio. De
acuerdo a los lineamientos internacionales, el único agente causal
de faringitis bacteriana es el estreptococo beta hemolítico del grupo
A (Streptococcus pyogenes), aunque también se han reportado
casos por estreptococo beta hemolíticos del grupo C y G. Las
bacterias que causan formas específicas de faringitis como
Corynebactarium diphtheriae, Bordetella pertusis, Neisseria
gonorrhoeae, anaerobios (angina de Vincent), no se aíslan por este
cultivo.';
                      break;
                    case 'DSBHPOS':
                      $re1='De acuerdo a los lineamientos internacionales, el único
agente causal de faringitis bacteriana es el estreptococo beta-hemolítico
del grupo "A" (Streptococcus pyogenes), así como también estreptococo
beta-hemolítico del grupo C y G.

Las bacterias que causan formas especificas de faringitis como Bordetella
pertusis, Corynebacterium diphtheriae, Neisseria gonorrhoeae y
anaerobios (angina de Vicent) no se aíslan por éste cultivo.

No es necesaria la realización de pruebas de susceptibilidad a los
antimicrobianos, porque hasta la fecha no se conoce resistencia al
tratamiento de elección, que son las penicilinas. En caso de alergia a
penicilinas, el tratamiento recomendado es la eritromicina.';
                      break; 
                    default:
                      $re1=$row['valor'];
                      break;
                  }
                  $pdf->MultiCell(120,5,utf8_decode(trim($re1)),0,'L');
                  break;
