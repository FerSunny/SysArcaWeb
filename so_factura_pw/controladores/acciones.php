...
class Acciones extends Query
{
	function Agregar()
	{
		...
		//Buscar Factura CE
		$query = "SELECT * FROM pw_factura WHERE id_factura = ?";
	...
	function Eliminar()
	{
		...
		$query = "UPDATE pw_factura SET status = 'S' WHERE id_factura = ?";
		$stmt = $conexion->prepare($query);
		$stmt->bind_param("i",$id);
	...
	function Terminar($last_id,$id)
	{
		...
		$query = "UPDATE pw_factura SET status = 'A' WHERE id_factura = ?";
		$stmt = $conexion->prepare($query);
		$stmt->bind_param("i",$id);
