...
// rechazo de muestras.
					{
						render:function(data,type,row){
							estatus=row['check_in_area'];
							rechazo=row['fk_id_rechazo_area'];
//console.log(estatus)
							if (rechazo > "0" ) {
								return "<button  disabled type='button' class='editar btn btn-danger btn-md'><i class='fas fa-ban'></i></button>";
							}else{
								if(estatus == 2){
									return "<button disabled  type='button' class='editar btn btn-danger btn-md'><i class='fas fa-ban'></i></button>";
								}else{
									return "<button   type='button' class='editar btn btn-warning btn-md'><i class='fas fa-check'></i></button>";	
								}
					...
					success: function(data)            

						{
...

							{
								...
								//windows.open('./reports/list_work.php')
...
var editar = function(tbody, table) {
...

		{
				...
				$("#frmedit  #dc").val(data.id_orden)

				$("#frmedit  #pro").val(data.id_orden)

				$("#frmedit  #codigo").val(data.id_orden)

				$("#frmedit  #email_sucursal").val(data.tulye)
				$("#frmedit  #email_sucursal").val(data.tlahuc)
				$("#frmedit  #email_sucursal").val(data.gregorio)
				$("#frmedit  #email_sucursal").val(data.xochimilco)
				$("#frmedit  #email_sucursal").val(data.dino)
				$("#frmedit  #email_sucursal").val(data.santiago)
				$("#frmedit  #email_sucursal").val(data.pablo)
				$("#frmedit  #email_sucursal").val(data.pedro)
				$("#frmedit  #email_sucursal").val(data.milpa)
				$("#frmedit  #email_sucursal").val(data.tecomitl)
				$("#frmedit  #email_sucursal").val(data.tetelco)
