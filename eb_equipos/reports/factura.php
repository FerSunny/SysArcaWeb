...
// Cabecera de página
  function Header()
  {
    ...
    $this->Cell(88);
    $this->Cell(5,5,utf8_decode($edad.', '.$sexo.', Tel.: '.$telefono.' rfc:'.$rfc_p),0,0,'L');
    ...
    $this->Ln(5);
    $this->Cell(5,5,utf8_decode('Solicitio: '.$medico),0,0,'L');
    $this->Cell(65);
    ...
    $this->Cell(1);
    $this->SetFont('Arial','B',9);
    $this->Cell(5,4,utf8_decode('Datos Clinicos: '),0,0,'L');
...
*/
    ...
    $this->Ln(4);
    $this->Cell(3);
    $this->Cell(190,5,'www.laboratoriosarca.com.mx',0,0,'C');
...
// verificamos si el estudio es paquete, para desglosarlo.
              if($row['per_paquete'] == 'Si'){
                $sql_pq="SELECT p.fk_id_estudio ,
                            es.`desc_estudio`
                        FROM km_paquetes p
                        LEFT OUTER JOIN km_estudios es ON (es.`id_estudio` = p.`fk_id_estudio`)
                        WHERE p.`id_paquete` =".$row['fk_id_estudio'];
              ...
              if ($row['fk_id_promosion']<>7){
                ...
                $sql_p="SELECT id_promocion , CONCAT('Aplica promocion (',porcentaje,'%) ',desc_promocion) AS promocion ,   porcentaje ,   fecha_inicio ,
                        fecha_final , lunes , martes , miercoles , jueves , viernes , sabado , domingo , tuly , tuly2 , greg , xochi , sant , pablo , pedro , teco , tete , dino, tla, estado ,DAYNAME('".$fecha_system."') as dia_semana
                        FROM  kg_promociones 
