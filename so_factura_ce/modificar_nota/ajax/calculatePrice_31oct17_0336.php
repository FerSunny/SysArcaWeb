...
function obtenerPrecioConPromocion($precio,$promocion) {
  ...
  return $resultado_precio_menos_promocion;
}
...
for($i=0; $i<count($data['ids']); $i++) {
    ...
    //aplicando promocion
      $sqlPromocionXEstudio="select porcentaje,fecha_inicio,fecha_final,lunes,martes,miercoles,jueves,viernes,sabado,domingo,tuly,tuly2,greg,xochi,sant,pablo,pedro,teco,tete,estado from  kg_promociones
      ...
      if(empty($id_factura)){
        ...
        $sqlGetDateBill="SELECT fecha_factura FROM so_factura where id_factura=".$id_factura;
...
//echo 'dia semana '.date('w', strtotime($fechaSpliteada[2]));
        //date('w', strtotime($fechaSpliteada[2]) obtiene el dia de la semana 
        switch(date('w', strtotime($fechaSpliteada[2]))){
          case 5:
           // echo 'martes';
