...
if ($con)
{
	...
	//$fetch = mysqli_query($con,"SELECT * FROM so_clientes where nombre like '%" . mysqli_real_escape_string($con,($_GET['q'])) . "%' and a_paterno  like '%" .  mysqli_real_escape_string($con,($_GET['q'])) . "%'");
		$fetch = mysqli_query($con,"SELECT id_cliente,a_paterno,a_materno,nombre,telefono_fijo,anios AS mail FROM so_clientes where activo = 'A' AND CONCAT_WS(' ', nombre, a_paterno,a_materno)  like '%" . mysqli_real_escape_string($con,($_GET['q'])) . "%' or a_paterno  like '%" . mysqli_real_escape_string($con,($_GET['q'])) . "%' or a_materno like '%" . mysqli_real_escape_string($con,($_GET['q'])) . "%';
");
