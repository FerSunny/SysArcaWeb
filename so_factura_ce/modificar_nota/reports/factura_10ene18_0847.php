...
// columna izquierda
            ...
            $pdf->SetXY(31, 47); 
            $pdf->Write(0,strtoupper(utf8_decode($row['nombre'])));
...
//echo $sql;
    ...
    if ($resultadoEstudios = mysqli_query($con, $sql)) {
        while($row = $resultadoEstudios->fetch_assoc())
        {
            ...
            if (strlen($row['desc_indicaciones'])>0){
                $linea_estudios=$linea_estudios+5;
                $pdf->SetFont('Arial','B', 9);
                $pdf->SetXY(30,$linea_estudios);
                $pdf->Write(0,'Indicaciones: '.utf8_decode(substr($row['desc_indicaciones'],0,48)));
            }
        ...
        $linea_estudios=$linea_estudios+8;
    }   
    ...
    //$pdf->SetXY(75, 230); 
    //$pdf->Write(0,utf8_decode('"En 30 días naturales, la empresa no se hace responsable de los estudios no recogidos"'));
