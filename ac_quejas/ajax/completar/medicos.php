...
if ($con)
{
	...
	//$fetch = mysqli_query($con,"SELECT * FROM so_clientes where nombre like '%" . mysqli_real_escape_string($con,($_GET['q'])) . "%' and a_paterno  like '%" .  mysqli_real_escape_string($con,($_GET['q'])) . "%'");
		$fetch = mysqli_query($con,"SELECT 
			...
			`activado`,
			`fk_id_perfil`
			FROM so_medicos c where c.estado = 'A' 
			AND (CONCAT_WS(' ', c.nombre, c.a_paterno,c.a_materno)  like '%" . mysqli_real_escape_string($con,($_GET['q'])) . "%' or c.a_paterno  like '%" . mysqli_real_escape_string($con,($_GET['q'])) . "%' or c.a_materno like '%" . mysqli_real_escape_string($con,($_GET['q'])) . "%');
");
