...
$sql = "SELECT concat(nombre,' ',a_paterno,' ',a_materno) as nombre FROM so_medicos WHERE concat(nombre,' ',a_paterno,' ',a_materno) LIKE (:keyword) ORDER BY nombre ASC LIMIT 0, 7";
...
foreach ($lista as $milista) {
    ...
    echo '<li onclick="set_item_m(\''.str_replace("'", "\'", $milista['nombre']).'\')">'.$nombre.'</li>';
}
?>