...
*/
					...
					
					{
						render:function(data,type,row){
									return "<form-group style='text-align:center;'>"+
									"<a id='printer' target='_blank' href='../ac_quejas_v2.0/reports/print_imagen_zoom.php?id_imagen="+row['id_imagen']+"&fk_id_queja="+row['fk_id_queja']+"' class='btn btn-dark btn-md' role='button'><i class='fa fa-eye' style='color: red;'></i></a>"+
									"</form-group>";	
									}
					}
		...
		var obtener_data_editar = function(tbody, table){
			$(tbody).on("click", "button.editar", function(){
				...
				var id_imagen = $("#frmedit #idimagen").val( data.id_imagen)
					   desc_imagen = $("#frmedit #edit1").val( data.fk_id_queja)
					   desc_estudio = $("#frmedit #fi_desc_estudio").val( data.fecha_registro)
						nombre = $("#frmedit #fi_nombre").val( data.nombre)
						document.getElementById("fi_imagen").src = "../ac_quejas_v2.0/img_queja/"+data.fk_id_queja+"/"+data.nombre
