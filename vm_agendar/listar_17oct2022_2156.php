...
-- LEFT OUTER JOIN vw_record_notas_medico ra5 ON (ra5.`fk_id_medico` = me.`id_medico` AND ra5.`anio_factura` = ( CASE WHEN MONTH(NOW()) < MONTH(DATE_SUB(NOW(),INTERVAL 5 MONTH)) THEN YEAR(DATE_SUB(NOW(),INTERVAL 1 YEAR)) ELSE YEAR(NOW()) END ) AND ra5.`mes_factura` < MONTH(DATE_SUB(NOW(),INTERVAL 5 MONTH)))
