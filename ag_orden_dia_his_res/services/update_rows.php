...
foreach($data as $objeto){
    ...
    if($flag!=true){
        $sql_delete_rows="delete from cr_plantilla_ekg_re where fk_id_factura=".$id_factura." and fk_id_estudio=".$id_studio;
        $execute_query_delete = mysqli_query($con,$sql_delete_rows);
        $flag=true;
    }

    $sql="INSERT INTO cr_plantilla_ekg_re(fk_id_empresa,fk_id_factura,fk_id_estudio,orden,tipo,concepto,valor,observaciones,num_imp,tamfue,tipfue,posini) VALUES (1,'$id_factura','$id_studio','$orden','$tipo','$concepto','$resultado','$observaciones',0,'$tamfue','$tipfue','$posini')";
    //echo $sql;
