...
// mail medico
					{
						render:function(data,type,row)
						{
							...
							if(perfil == 1 || perfil == 42 || perfil == 40 || perfil == 33 || perfil == 45 || perfil == 46)
							{

									if(resta == 0)
									{
										//console.log('aqui')
										return "<button type='button' id='e_medico' class='btn btn-primary btn-md'><i class='fas fa-file-pdf'></i></button>"
									}else
									{
		...
		var email_medico = function(tbody, table){
			$(tbody).on("click", "button#e_medico", function(){
				...
				if(registro == 'Si')
				{
					...
					if(plantilla == 7)
					{
						window.open('../ag_orden_dia_usg/reports/print_result.php?numero_factura='+fac+'&studio='+est, '_blank');
					...
					if(plantilla == 6)
					{
						window.open('../ag_orden_dia_rx/reports/print_result.php?numero_factura='+fac+'&studio='+est, '_blank');
						/*
					...
					if(plantilla == 5)
					{
						window.open('../ag_orden_dia_ekg/reports/print_result.php?numero_factura='+fac+'&studio='+est, '_blank');
						/*
					...
					*/
					if(plantilla == 3)
					{
						window.open('../ag_orden_dia_pacientes/ag_orden_dia/reports/print_result_p3.php?numero_factura='+fac+'&studio='+est, '_blank');
						/*
					...
					if(plantilla == 2)
					{
						window.open('../ag_orden_dia_pacientes/ag_orden_dia/reports/print_result_p2.php?numero_factura='+fac+'&studio='+est, '_blank');
					...
					if(plantilla == 1)
					{
						...
						window.open('../ag_orden_dia_pacientes/ag_orden_dia/reports/print_result_p1.php?numero_factura='+fac+'&studio='+est, '_blank');
		...
		var email_paciente = function(tbody, table){
			$(tbody).on("click", "button#e_paciente", function(){
				...
				if(registro == 'Si')
				{
					...
					{
						$.post("./reports/print_result_mail_6.php",{'numero_factura': fac, 'studio' : est },function(data, status){
							$.post("./services/send_mail_paciente.php",{'numero_factura': fac, 'studio' : est, 'enviar' : enviar},function(data, status){
								console.log('Se envio email: ' + data)
								if(data == 1)
								{
									Swal.fire('Mensaje enviado')
