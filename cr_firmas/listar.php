<?php
  ...
  if ($fk_id_perfil==1 or $fk_id_perfil==13 or $fk_id_perfil==40) 
    {
      $condicion=' > 0';
      $id_usuario=' > 0';
    }
...
WHERE pq.estado = 'A'
  ...
  AND DATE(fa.fecha_entrega) BETWEEN DATE_SUB(CURDATE(), INTERVAL 100 DAY) AND DATE_ADD(CURDATE(), INTERVAL 10 DAY)
  AND df.fk_id_estudio IN (SELECT DISTINCT fk_id_estudio FROM cr_plantilla_esp WHERE estado = 'A')
  AND fa.fk_id_sucursal ".$condicion."
 ) res
LEFT OUTER JOIN cr_plantilla_esp_re p2 ON (p2.fk_id_factura = res.id_factura AND p2.fk_id_estudio = res.fk_id_estudio)
...
LEFT OUTER JOIN cr_plantilla_esp_re p2 ON (p2.fk_id_factura = df.id_factura AND p2.fk_id_estudio = df.fk_id_estudio),
km_estudios es 
...
AND df.fk_id_estudio IN (SELECT DISTINCT fk_id_estudio FROM cr_plantilla_esp WHERE estado = 'A')
AND DATE(fa.fecha_entrega) BETWEEN DATE_SUB(CURDATE(), INTERVAL 150 DAY) AND DATE_ADD(CURDATE(), INTERVAL 10 DAY)
AND fa.fk_id_sucursal ".$condicion
;
//-- 
//echo $query;
