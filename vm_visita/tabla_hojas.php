<?php
  ...
  {
   $fk_id_medico=$_GET['numero_factura'];
   $_SESSION['fk_id_medico']=$fk_id_medico;
   $nombre='';
   $query="SELECT CONCAT(me.nombre,' ',me.a_paterno,' ',me.`a_materno`) AS nombre FROM so_medicos me
         WHERE me.estado = 'A'
         AND me.`id_medico` = '$fk_id_medico'";
   $resultado = mysqli_query($conexion, $query);
   if($row = mysqli_fetch_array($resultado)){
     $nombre = $row['nombre'];
   }else
   {
     $nombre='error al extraer el nombre';
   }
   ;
?>
<!DOCTYPE html>
...
<head>
	...
	<title>Visita Medica</title> <!-- CAMBIO  Titulo de la forma -->
...
<body background="../imagenes/logo_arca_sys_web.jpg">
  ...
  include("formularios/formularios_hojas.php"); // CAMBIO programa de la forma
  ?>

  <div class="col-sm-12 col-md-12 col-lg-12">
      <h1>Tabla de Visitas medico  <!-- CAMBIO Se cambia el titulo de la tabla -->
          <button type="button" class="btn btn-primary pull-right menu" data-toggle="modal" data-target="#myModals">
           <i class="fa fa-user-plus" aria-hidden="true"></i>&nbsp;Nueva Hoja</button> <!-- CAMBIO Se cambia el boton de altas -->
      </h1>
      <h3>Turno: <?php echo '('.$fk_id_medico.') '.$nombre?>
  </div>
		<div class="row">
	...
	<script language="javascript" src="js/tabla_hojas.js"></script> <!-- CAMBIO este JS -->
	<script>
