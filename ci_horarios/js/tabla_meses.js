		...
		var listar = function(){
			...
			var table = $("#dt_productos").DataTable({
				...
				"columns":[
					...
					//{"defaultContent":"<button type='button' class='eliminar btn btn-success btn-md'><i class='fas fa-dragon'></i></button>"}

					{
						render:function(data,type,row){
							return "<form-group style='text-align:center;'>"+
							"<a id='printer'  href='../ci_horarios/tabla_horarios.php?fk_id_sucursal="+row['fk_id_sucursal']+"&anio="+row['anio']+"&mes="+row['mes_num']+"' class='btn btn-success btn-md'  role='button'><span class='far fa-bell' style='color:white;'></span></a>"+
							"</form-group>";
...
var editar = function(tbody, table) {
...

		{
				...
				$("#frmedit  #dc").val(data.id_horario)

				$("#frmedit  #pro").val(data.anio)

				$("#frmedit  #codigo").val(data.mes_num)

				$("#frmedit  #sucursal").val(data.fk_id_sucursal)

				$("#frmedit  #servicio").val(data.fk_id_servicio)

				$("#frmedit  #medico").val(data.fk_id_medico)

				$("#frmedit  #fecha").val(data.dia_atencion)

				$("#frmedit  #hinicio").val(data.hora_inicio)

				$("#frmedit  #hfinal").val(data.hora_final)

				$("#frmedit  #subrrogado").val(data.subrrogado)

				$.post("./select/select_servicios.php?val=1", {'id_servicio' : data.fk_id_servicio, 'id_municipio' : data.fk_id_medico}  , function(res,status)
				{
					console.log(res)
					$("#frmedit #medico").html(res);
					$("#frmedit #medico option[value='"+data.fk_id_medico+"']").attr("selected",true);
...
$("#frmedit").on('submit', function (e) 

	{
			...
			$.ajax({
					...
					url: "controladores/duplica_mes.php",                    
		...
		$(tbody).on("click", "button.eliminar", function() {
			...
			swalWithBootstrapButtons({
				...
				if (result.value) {
					 ...
					 $.post("./controladores/eliminar.php", {'id_producto' : data.id_horario}  , function(data,status)
...
// tomar el evento de servicios
		$("#myModals select[name=servicio]").change(function()
		{

				select = $('#myModals select[name=servicio]').val();
				//alert(select)
				...
				$.ajax({
					type: "POST",
					url: "./select/select_servicios.php?val=1",
					data:parametros ,
					beforeSend: function(){
					},
					success: function(data)
