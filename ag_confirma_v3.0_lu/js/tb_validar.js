...
var listar = function() {
        ...
        var table = $("#dt_resultados").DataTable({
            ...
            "columns": [
                ...
                //{ "defaultContent": "<button type='button' class='print btn btn-danger btn-md' style='background-color: #DF0101 !important;'><i class='fas fa-file-pdf'></i></button>" },
                {
                    render: function(data,type,row)
                    {
                        ...
                        if(valid == 1)
                        {
                            return "<button type='button' class='print btn btn-success btn-md'><i class='fas fa-file-pdf'></i></button>"
                        }else
                        {
                            return "<button type='button' disabled class='print btn btn-danger btn-md'><i class='fas fa-file-pdf'></i></button>"
                        }
                    }
                },
...

                {
                    render: function(data,type,row)
                    {
                        ...
                        if(valid == 0)
                        {
                            return "<button type='button' class='valid btn btn-success btn-md'><i class='fas fa-check'></i></button>"
                        }else
                        {
                            return ""
                        }
                ...
                //{ "defaultContent": "<button type='button' class='window btn btn-primary btn-md'><i class='fas fa-edit'></button>"}
           
                {
                    render: function(data,type,row)
                    {
                        ...
                        if(valid == 1)
                        {
                            return "<button type='button' class='window btn btn-success btn-md'><i class='fas fa-edit'></i></button>"
                        }else
                        {
                            return "<button type='button' disabled class='window btn btn-danger btn-md'><i class='fas fa-edit'></i></button>"
                        }
                    }
                },           
...
console.log(estudio+'=estudio');
        if(plantilla == 1)
        {
            if(estudio == 235 || estudio == 2460 || estudio == 2616 || estudio == 2675)
            {
                console.log('reporte entro');
...
var modificar = function(tbody, table) {
    ...
    if(plantilla == 1)
    {
        if(estudio == 235 || estudio == 2675)
        {
            window.open('./plantilla_1_3h/frm_update.php?cliente='+data.fk_id_cliente+'&factura='+data.fk_id_factura+'&estudio='+data.fk_id_estudio, '_blank');

        }else
        	if(estudio == 236)
        	{
            	window.open('./plantilla_1_5h/frm_update.php?cliente='+data.fk_id_cliente+'&factura='+data.fk_id_factura+'&estudio='+data.fk_id_estudio, '_blank');
    ...
    if(plantilla == 3)
    {
        window.open('./plantilla_3/frm_update.php?cliente='+data.fk_id_cliente+'&factura='+data.fk_id_factura+'&estudio='+data.fk_id_estudio, '_blank');
    }else
    {
