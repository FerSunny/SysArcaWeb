...
$comentarios="";
	...
	$stmt = $conexion->prepare("SELECT valor,verificado,observaciones FROM cr_plantilla_2_re WHERE fk_id_factura = ? AND fk_id_estudio = ? AND tipo = ?");

    $stmt->bind_param('iis', $id_factura,$id_estudio,$tipo);
	$stmt->execute();
