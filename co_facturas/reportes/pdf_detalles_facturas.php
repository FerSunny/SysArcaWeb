...
// Pie de página
  function Footer()
  {
    ...
    function Tabla()
    {
    	...
    	global $conexion,$usuario,$nombre;
        ...
        $query = "SELECT fa.id_factura,suc.desc_sucursal,DATE_FORMAT(fa.fecha_factura, '%d de %M de %y') f_factura,
                ...
                AND fa.fk_id_tipo_pago = ? AND fa.grupo = ? AND fa.requiere_factura = 1";
        //Fecha en español
        ...
        while($row = $result->fetch_assoc())
	    {
           ...
           $this->Cell(60,5,$row['f_factura'],1,0,'C');
           $this->Cell(20,5,'$ '.$row['imp_total'],1,0,'R');
           $this->Cell(20,5,'$ '.$row['a_cuenta'],1,0,'R');
           $this->Cell(20,5,'$ '.$row['resta'],1,0,'R');
	       $this->ln();
	    }
        $query = "SELECT SUM(imp_total) total, SUM(a_cuenta) cuenta, SUM(resta) resta FROM so_factura fa
WHERE DATE(fa.fecha_factura) >= ? AND DATE(fa.fecha_factura) <= ?
AND fa.fk_id_tipo_pago = ? AND fa.grupo = ? AND fa.requiere_factura = 1";
        $stmt = $conexion->prepare($query);
        $stmt->bind_param("ssis",$f_inicio,$f_final,$tp,$grupo);
