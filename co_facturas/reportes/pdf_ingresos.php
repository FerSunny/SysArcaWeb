  ...
  function Footer()
  {
    ...
    function Tabla()
    {
			...
			while($row = $result->fetch_assoc())
	    {
	    	//Titulo
	    	$this->SetFont('Arial','B',16);
	    	$this->Cell(190,0,utf8_decode('Reporte por razón social: '.$row['desc_grupo']),0,0,'C');
	    	$this->ln(10);
	    }
        ...
        $this->ln(5);
        $this->SetFont('Arial','',10);
        $this->Cell(190,0,'Tipo: Ingresos',0,0,'L');
        $this->ln(10);
    	//Titulos tabla
    	$this->SetFont('Arial','B',10);
      ...
      $query = "SELECT '1' tipo,'1' tipo_pago, SUM(fa.imp_total) total,SUM(fa.a_cuenta) cuenta,SUM(fa.resta) resta,COUNT(*) folios FROM so_factura fa
			WHERE DATE(fa.fecha_factura) >= ? AND DATE(fa.fecha_factura) <= ? AND fa.fk_id_tipo_pago = 1 AND fa.grupo = ?
			...
			WHERE DATE(fa.fecha_factura) >= ? AND DATE(fa.fecha_factura) <= ? AND fa.fk_id_tipo_pago = 2 AND fa.grupo = ?
			...
			WHERE DATE(fa.fecha_factura) >= ? AND DATE(fa.fecha_factura) <= ? AND fa.fk_id_tipo_pago = 3 AND fa.grupo = ?
			...
			WHERE DATE(fa.fecha_factura) >= ? AND DATE(fa.fecha_factura) <= ? AND fa.fk_id_tipo_pago = 4 AND fa.grupo = ?
			...
			WHERE DATE(fa.fecha_factura) >= ? AND DATE(fa.fecha_factura) <= ? AND fa.fk_id_tipo_pago = 5 AND fa.grupo = ?";
      $stmt = $conexion->prepare("SET lc_time_names = 'es_ES'");
			...
			while($row = $result->fetch_assoc())
	    {
	      ...
	      $this->ln();
	    	...
	    	$stmt = $conexion->prepare("SELECT COUNT(*) folios,SUM(fa.imp_total)total,SUM(fa.a_cuenta) cuenta,SUM(fa.resta) resta,COUNT(*) folios
					FROM so_factura fa
					WHERE DATE(fa.fecha_factura) >= ? AND DATE(fa.fecha_factura) <= ? AND fa.grupo = ?");
	    $stmt->bind_param("sss",$f_inicio,$f_final,$grupo);
			$stmt->execute();
			$result = $stmt->get_result();
			if($result->num_rows === 0) exit('No hay documentos proximos');
			while($row = $result->fetch_assoc())
	    {
	      ...
	      $this->Cell(20,5,$row['folios'],1,0,'C');
	      $this->Cell(30,5,'$ '.$row['total'],1,0,'C');
	      $this->Cell(30,5,'$ '.$row['cuenta'],1,0,'C');
	      $this->Cell(30,5,'$ '.$row['resta'],1,0,'C');
	      $this->ln();
	    }
    }
  }
