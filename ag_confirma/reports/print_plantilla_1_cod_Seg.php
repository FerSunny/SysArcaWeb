...
// Tercer linea
    ...
    $this->Cell(70,4,'Codigo de seguridad ARCA',0,0,'L');
    $this->ln(3);
...
// Pie de página
  ...
  function Footer()
  {
    global $studio,$con,$verificado,$tamfuev,$tipfuev,$posiniv,$numero_factura;
      ...
      $this->Cell(30,40,$verificado,0,0,'L'); 
      $this->ln(30); // aqui
    //$this->Cell(5);
      ...
      while($row = $result->fetch_assoc())
        {
          ...
          $firma=$row['concepto'];
          $this->Image('../imagenes/firma.gif',153,224,40,0);
   
       $this->SetFont('Arial','',$row['tamfue']);
          $this->Cell(170,5,utf8_decode($firma),0,0,'L');
          $this->ln(4);//para juntar el texto 
        }

    $this->ln(-1);//para subir y bajar la linea
    $this->Cell(4);
    $this->SetTextColor(0,0,255);
    $this->Cell(18,5,'_____________________________________________________________________________________________________________________________________',0,0,'L');    
