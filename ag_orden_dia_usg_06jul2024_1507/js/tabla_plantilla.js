...
// listar datos en la tabla de perfiles
		function listar(){
			var table=$('#dt_agenda_p').DataTable({
				processing: true,
				serverSide: false,
				lengthMenu: [10, 25, 50],
				select: true,
				...
				"columns":[
					...
					{"data" : "titulo_desc"},
					{"data" : "desc_corta"},
					{"defaultContent" : "<button type='button' class='editar btn btn-primary btn-md' data-toggle='modal' data-target='#modalEditar'><i class='fa fa-pencil-square-o'></i></button>"}
			...
			obtener_data_editar("#dt_agenda_p tbody", table);
...
// editamos perfiles
		var obtener_data_editar = function(tbody, table){
			$(tbody).on("click", "button.editar", function(){
				...
				$("#frmedit #idplantilla").val( data.id_plantilla );
				$("#frmedit #fi_id_plantilla").val( data.id_plantilla );
				$("#frmedit #fi_nombre_plantilla").val( data.nombre_plantilla);
