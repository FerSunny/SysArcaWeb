...
$sql="SELECT fa.`id_factura`, fa.estado_factura, 
CASE 
...
fa.`imp_total`, 
(fa.`a_cuenta`- IFNULL(pa.imp_pagado,0)) AS a_cuenta, 
CASE 
WHEN pa.imp_pagado IS NULL THEN
fa.`resta`
ELSE
pa.imp_pagado
END AS resta, 
...
END AS estado_pago
FROM so_factura fa
LEFT OUTER JOIN so_pagos pa ON (pa.fk_id_factura = fa.id_factura AND DATE(pa.fecha_pago) = DATE(fa.fecha_factura))
LEFT OUTER JOIN kg_sucursales su ON (su.id_sucursal = pa.fk_id_sucursal_pag),
kg_tipo_pago tp,so_clientes cl
  ...
  while($row = $result->fetch_assoc())
    {
      ...
      $pdf->Cell(13,5,$row['suc_ori'],0,0,'L');
      $pdf->Cell(15,5,'$'.number_format($row['imp_total'],2),0,0,'R');
