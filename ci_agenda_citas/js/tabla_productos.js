				...
				"columns":[
...

					{
						render:function(data,type,row){
							var paciente=row['fk_id_paciente']
							var id_factura=row['id_factura']
							if (paciente > 0 && id_factura == null){

								return "<form-group style='text-align:center;'>"+
								"<a id='printer' target='_blank' href='../so_factura/facturas.php?cliente="+row['id_cliente']+"' class='btn btn-outline-success btn-sm'  role='button'>Hacer notra</a>"+
								"</form-group>";

							}else{
								return '<button type="button" disabled=true class="btn btn-outline-info btn-sm">Atendido</button>'
							}
						}
					},
...

					{
						render:function(data,type,row){
							var paciente=row['fk_id_paciente']
							var id_factura=row['id_factura']
							if (paciente > 0 && id_factura >0){
								return "<button type='button' disabled=true class='eliminar btn btn-outline-danger btn-sm'>No aplica</button>"
							}else{
								return "<button type='button' class='eliminar btn btn-outline-danger btn-sm'>Cancelar</button>"
							}
						}
					}
					...
					success: function(data)            

						{
...

							{
								...
								swal('Ya ha sido creado el dia de trabajo')
...
var editar = function(tbody, table) {
...

		{
				...
				$("#frmedit  #tiempo").val(data.tiempo)
		...
		$(tbody).on("click", "button.eliminar", function() {
			...
			swalWithBootstrapButtons({
				...
				if (result.value) {
					 ...
					 $.post("./controladores/eliminar.php", {'id_producto' : data.id_evento}  , function(data,status)
