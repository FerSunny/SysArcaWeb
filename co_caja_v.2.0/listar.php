<?php
  ...
  $query = "SELECT ".$fk_id_perfil." as id_perfil, 
  us.iniciales,   
  fa.fk_id_usuario,
  su.`desc_sucursal` AS sucursal, 
  su.id_sucursal, date(fa.fecha_factura) as fecha_factura,
...
FROM so_factura fa
LEFT OUTER JOIN co_cortes co ON (co.fk_id_sucursal = fa.fk_id_sucursal AND co.fk_id_usuario = fa.fk_id_usuario AND co.fecha_cierre = DATE(fa.fecha_factura)),
     kg_sucursales su,
     se_usuarios us
WHERE DATE_SUB(CURDATE(), INTERVAL ".$diasatras." DAY) <= DATE(fecha_factura)
  ...
  AND us.id_usuario = fa.fk_id_usuario
GROUP BY 
us.iniciales, 
fa.fk_id_usuario,
su.`desc_sucursal`,
