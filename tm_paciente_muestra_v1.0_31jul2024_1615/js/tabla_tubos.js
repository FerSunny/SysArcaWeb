...
// listar datos en la tabla de perfiles
		var listar = function(){
					...
					},
					{
						render:function(data,type,row){
							...
							if (v_aplico == 'N'){
								...
								return "<button type='button' class='editar1 btn btn-warning btn-md'> <i class='fas fa-thumbs-down'></i></button>"

							}else{
								return "<button type='button' class='editar1 btn btn-warning btn-md' disabled> <i class='fas fa-thumbs-down'></i></button>"
							}
						},
					}
					/*
			...
			editar1("#dt_productos tbody", table)
...
/*							
							switch (data) {
								case 1:
									...
									break;
								case 1062:
									swal('Los valores registrados ya existe, recargue la pagina')
									break;
								default:
...
*/
							...
							{
								if(data== 1062){
									swal('La muestra YA ha sido registrada')
								}else{
									swal('Error en MySQL, Error numero:  '+ data)
									console.log(data)
								}
					...
					success: function(data)            
						{
							...
							if(data==1)
							{
								var table = $('#dt_productos').DataTable(); // accede de nuevo a la DataTable.
								table.ajax.reload(); // Recarga el  DataTable
								swal('Muestra no recibida. Sera necesario agendar una nueva entrega')
								console.log(data)
							}
							else
							{
...
$("#frmedit").on('submit', function (e) 
{
		...
		$.ajax({
				type: "POST",                 
				url: "controladores/edita_agenda.php",                    
