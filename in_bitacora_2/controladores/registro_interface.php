...
{ // inicio de while
	...
	// LDL COLESTEROL (LDL CAL=<COL>-<HDL-C>-(<TG2>/5))
			...
			if ($valor_regresa > 0.0) {
				...
				if($valor_regresa_hdlc > 0.0){
						...
						if($valor_regresa_tg > 0.0){
							$v_valor = ($valor_regresa-$valor_regresa_hdlc)-($valor_regresa_tg/5);
						}else{
	...
	// I/A RIESGO INDICE ETEROGENICO I/A=<COL>/<HDL-C>
	...
	// MAN **** SE COLOCA EL VAOR NORMAL *** para aspecto del suero
		}elseif ($row['codigo_int'] == 'MAN') {

				$v_valor = 99.9; //round(($valor_regresa/5),1);



	
	// FOSF FOSFOLIP *** Formula  --> (FOSF=0.766*<COL>+62.3)
		}elseif ($row['codigo_int'] == 'FOSF') {
			$valor_regresa=busca_tester($fecha_test, $id_factura ,'COL');
			if ($valor_regresa > 0.0) {
				$v_valor = round((.766*$valor_regresa+62.3),1);
			}else{
				$resu = insert_log($fk_id_sucursal, $fk_id_empresa,$id_factura,$busca_estudio,'TG2');
				$v_valor = $valor_regresa;
			}
	...
	// LIPIDOS TOTALES (LIPT=.2.27*<COL>+<TG2>+62.3)
			// Aumentado 13feb2023 by JPM 
		}elseif ($row['codigo_int'] == 'LIPT') {
			$valor_regresa=busca_tester($fecha_test, $id_factura ,'COL');
			if ($valor_regresa > 0.0) {
				$v_valor_col = $valor_regresa;
				$valor_regresa=busca_tester($fecha_test, $id_factura ,'TG2');
				if ($valor_regresa > 0.0) {
					$v_valor_tg2 = $valor_regresa;
					$v_valor = 2.27*$v_valor_col+$v_valor_tg2+62.3;
				}else{
					$resu = insert_log($fk_id_sucursal, $fk_id_empresa,$id_factura,$busca_estudio,'TG2zz');
					$v_valor = $valor_regresa;
				}	
			}else{
				$resu = insert_log($fk_id_sucursal, $fk_id_empresa,$id_factura,$busca_estudio,'COLzz');
				$v_valor = $valor_regresa;
			}
...
function busca_tester($fecha_test, $id_factura , $codigo_int)
{
	...
	case 
		...
		when te.tes_testname in ('GLU','COL','TG2') then
			FORMAT(ROUND(te.tes_result,0),1)
		when te.tes_testname in ('HDL-C') then
			ROUND(CAST(te.tes_result AS DECIMAL(4,1)))
		else
