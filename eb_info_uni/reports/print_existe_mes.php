...
AND so.fk_id_sucursal = ".$fk_id_sucursal."
and so.tipo = 2
AND so.estado = 'A'
AND so.estatus = 'I'
...
AND ins.`fk_id_producto` = ac.`fk_id_producto`
AND YEAR (fa.`fecha_factura`) = YEAR(ac.`fecha_actualizacion`)
AND MONTH(fa.`fecha_factura`) = 8) AS conumos, 


-- EXISTENCIA ACTUAL
	(ac.`existencias` - (SELECT IFNULL(SUM(df.cantidad),0) AS consumo
FROM 	so_detalle_factura df,
	so_factura fa,
	km_insumos_estudio ins
WHERE fa.fk_id_sucursal = ".$fk_id_sucursal."
...
AND ins.`fk_id_producto` = ac.`fk_id_producto`
AND YEAR (fa.`fecha_factura`) = YEAR(ac.`fecha_actualizacion`)
AND MONTH(fa.`fecha_factura`) = 8) ) as existencias
  ...
  while($row = $result->fetch_assoc())
    {
      ...
      $pdf->Cell(25,5,utf8_decode($row['desc_producto']),0,0,'L');
	  $pdf->Cell(20);
      $pdf->Cell(8,5,utf8_decode($row['abreviatura']),0,0,'L');
	  $pdf->Cell(1);    
