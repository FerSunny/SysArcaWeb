...
SELECT fa.id_factura,
 ...
 /*
 case
  when es.id_estudio = 2699 then
    substr(es.desc_estudio,5,145) 
  else
    es.desc_estudio
  end as estudio2,
  */
  ...
  CASE
  WHEN SUBSTR(es.desc_estudio,1,4) = 'MC -' THEN
  REPLACE(es.desc_estudio,'MC -','')
  WHEN SUBSTR(es.desc_estudio,1,3) = 'L -' THEN
  REPLACE(es.desc_estudio,'L -','')
  WHEN SUBSTR(es.desc_estudio,1,4) = 'TC -' THEN
  REPLACE(es.desc_estudio,'TC -','') 
  ELSE
  es.desc_estudio
  END AS estudio2,
...
if ($result = mysqli_query($con, $sql)) {
  while($row = $result->fetch_assoc())
    {
      if($row['tipo']=='M'){
        if($studio == 117 and $numero_factura == 261448 ){
          $metodo='Metodo: TURBIDIMETRIA';
        }else{
          $metodo=$row['concepto'];
        }
...
function Header()
{
    ...
    $this->Image('../imagenes/logo_arca.png',10,5,40,0);
    $this->Image('../imagenes/pacal.jpg',160,7,25,0);
    $this->Image('../imagenes/codigo1.jpg',170,35,20,20);
...
// quitar dos lineas para el estudio 410 (quimica de 12 elementos)
    if ($studio=='410' or $studio=='966' or $studio=='1051' or $studio=='1776' or $studio=='411'){
      $this->ln(12);
    }else{
      $this->ln(13);
    }
...
// linea de encavezados (metodo)
    ...
    switch ($studio) {
      ...
      case '476': //PERFIL TIROIDEO BASICO I
          //$this->Ln(10);
		  $this->Ln(-19); // se cambio para subir los titulos pedidos por dmom 17ene2023
...
// Pie de página
  function Footer()
  {
    ...
    $this->Cell(5);
    $this->SetTextColor(0,0,255);
    $this->Cell(185,5,'_____________________________________________________________________________________________________________________________',0,0,'L');
...
SELECT 	a.id_factura,
	...
	WHEN a.id_factura = 242841 AND a.fk_id_estudio = 742 AND a.orden = 56.50 THEN
  	...
  	-- WHEN a.tipo ='T' AND (LENGTH(valor_solo) = 0 OR valor_solo IS NULL) AND a.fk_id_estudio ='2699' THEN
    -- ''
	ELSE
		a.concepto
	END AS concepto,
	...
	CASE
	WHEN a.id_factura = 284122 AND a.fk_id_estudio = 2583 AND a.orden = 56 THEN
		'33.0 - 193.0  µg/dL  MUJER'
		
	WHEN a.id_factura = 288737 AND a.fk_id_estudio = 2584 AND a.orden = 59 THEN
		'33.0 - 193.0  µg/dL  MUJER'
...
CASE
  WHEN fk_id_estudio in (274,963,988,69,459,1029,1717,2432,2577,186,402) THEN
        CONCAT(valor_refe) 
  ELSE
  ...
  (SELECT $numero_factura as id_factura,
  p2.fk_id_estudio,
  null,p2.tipo,p2.orden,p2.concepto,null,null,null,null,CASE
  WHEN fk_id_estudio in (274,963,988,1029,2432,2577,186,402) THEN
    CONCAT(valor_refe)
  ELSE
...
*/
        ...
        if($row['tipo']=='P' && strlen($row['valor_solo'])==0 and ($studio=='274' or $studio=='151' or $studio=='152' or $studio=='153' or $studio=='348' or $studio=='904' or $studio=='905' or $studio=='494' or $studio=='279' or $studio=='395' or $studio=='117' or $studio=='525' or $studio=='527' or $studio=='528' or $studio=='526' or $studio=='963' or $studio=='956' or $studio=='957' or $studio=='958' or $studio=='983' or $studio=='987' or $studio=='988' or $studio=='989' or $studio=='990' or $studio=='1027' or $studio=='1028' or $studio=='1029' or $studio=='202' or $studio=='1201' or $studio=='1194' or $studio=='1195' or $studio=='1196' or $studio=='1211' or $studio=='1226' or $studio=='1227' or $studio=='1204' or $studio=='1673' or $studio=='1830' or $studio=='1717' or $studio=='1854' or $studio=='1730' or $studio=='1961' or $studio=='1959' or $studio=='1962' or $studio=='1963' or $studio=='1747' or $studio=='1213' or $studio=='1766' or $studio=='1718' or $studio=='2151' or $studio=='2152' or $studio=='2153' or $studio=='2154' or $studio=='2427' or $studio=='2428' or $studio=='2429' or $studio=='2461' or $studio=='2432' or $studio=='2441' or $studio=='2524' or $studio=='2577' or $studio=='2596' or $studio=='2597' or $studio=='2598' or $studio=='2432' or $studio=='2617' or $studio=='2627' or $studio=='2609' or $studio=='2699' or $studio=='186' or $studio=='402')){
            $nle+=1;
            $nle-=1;
        }else{
             // $this->ln(5)
...
// termina valor
            ...
            //El if compara el estudio 234 y quita los valores de referencia
              $pdf->Cell(8);
              $pdf->SetFont('Arial',$row['tipfue'],$row['tamfue']);
   ...
   //echo '--'.$row['valor_refe'].'--';               
                 ...
                 // if($studio == '243' && $row['concepto'] == "Nitrogeno Ureico en Orina" || $row['concepto'] == "Volumen Referido (24h)" || $row['concepto'] == "Volumen Referido (24h)" or ($studio == 1180 and $row['concepto'] == 'Yodo Proteico') )
					if($studio == '243' && $row['concepto'] == "Nitrogeno Ureico en Orina" || $row['concepto'] == "Volumen Referido (24h)" || $row['concepto'] == "Volumen Referido (24h)" ) // se quito el estudio 1180 yodo proteico solicitado por dmolm 27ene20223
                  {
                      $pdf->Cell(18,5,"",0,0,'L');
                  }
                  else{
