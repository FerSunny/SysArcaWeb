...
// listar datos en la tabla de comisiones
		var listar = function(){
			...
			var table = $("#dt_especialidad").DataTable({
				"destroy":true,
				"sRowSelect": "multi",
				"ajax":{
					"method":"POST",
		...
		obtener_data_editar("#dt_especialidad tbody", table);
		obtener_id_eliminar("#dt_especialidad tbody", table);
		}
// editamos estado civil
		var obtener_data_editar = function(tbody, table){
			$(tbody).on("click", "button.editar", function(){
				var data = table.row( $(this).parents("tr") ).data();
				var idesp = $("#frmedit #idesp").val( data.id_especialidad),
					   desc_esp = $("#frmedit #edit1").val( data.desc_especialidad),
...
// eliminndo la comision
		var obtener_id_eliminar = function(tbody, table){
			$(tbody).on("click", "button.eliminar", function(){
				var data = table.row( $(this).parents("tr") ).data();
				var id_estado_civil = $("#frmEliminaredocivil #idesp").val( data.id_especialidad);
				 desc_estado_civil =$("#frmEliminaredocivil #esp").val(data.desc_especialidad);
