...
$sql="(select CONCAT(valor,' ',unidad_medida) as valor_solo,tipo,orden,concepto,concat(LPAD(trim(valor),15,' '),' ',unidad_medida,verificado) as valor,concat(unidad_medida,verificado) as unidad_medida,verificado,observaciones,
CASE
  WHEN fk_id_estudio in (274,963,988,69,459,1029,270) THEN
        CONCAT(valor_refe) 
  ELSE
...
//inicia el cocepto
    ...
    // termina
          ...
          if ($row['concepto'] == '.'){
            ...
            $pdf->Cell(2);
            $pdf->Cell(70,5,utf8_decode($row['concepto']),0,0,'J');
          }
              //$pdf->ln(5);
// Termina conecpto
...
// inicia valor          
          ...
          $pdf->Cell(26,5,utf8_decode($row['valor_solo']),0,0,'C');
          //$pdf->Cell(18,5,utf8_decode($row['unidad_medida']),0,0,'C');  
...
// termina valor
              ...
              $pdf->Cell(28);
              $pdf->SetFont('Arial',$row['tipfue'],$row['tamfue']);
              $pdf->Cell(18,5,utf8_decode($row['valor_refe']),0,0,'L');
