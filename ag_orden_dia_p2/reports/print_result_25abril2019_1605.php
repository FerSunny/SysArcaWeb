...
$sql="SELECT p2.tipo,p2.concepto,posini,tipfue,tamfue FROM cr_plantilla_2 p2 WHERE p2.fk_id_estudio =".$studio." AND p2.estado = 'A' AND p2.tipo in ('M','R')";
if ($result = mysqli_query($con, $sql)) {
  while($row = $result->fetch_assoc())
    {
      if($row['tipo']=='M'){
...
// Tercer linea
    ...
    $this->ln(3);
    $this->MultiCell(160,3,base64_encode($cadena),0,'L');
    $this->ln(-1);
...
// Pie de página
  function Footer()
  {
    ...
    $this->Cell(6);
    $this->SetTextColor(0,0,255);
    $this->Cell(18,5,'__________________________________________________________________________________________________________________________________',0,0,'L');
