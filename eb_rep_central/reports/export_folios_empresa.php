...
$sql="
SELECT 'Arca' AS sucursal, COUNT(id_factura) AS numfolios, 
...
/*
   ...
   //$this->SetFillColor(230,230,0);
    ...
    $this->Ln(1);
    $this->Cell(6);
    $this->Cell(185,5,'______________________________________________________________________________________________',0,0,'L');
    $this->Ln(5);
    */
    $this->Cell(6);
    $this->SetFont('Courier','B',12);
    //$this->Cell(180,5,'RELACION DE FOLIOS ASIGNADOS',0,0,'C');
    $this->SetTextColor(0,0,0);
/*
    $this->ln(3);
    $this->Cell(5);
    $this->SetFont('Arial','',13);
    $this->Cell(185,5,'_________________________________________________________________________',0,0,'L');
...
// titulos 
    ...
    $this->Cell(185,5,'LISTA DE FOLIOS',0,0,'C');
    ...
    $this->Cell(5,5,'#',0,0,'R');
    $this->Cell(12,5,'Folios',0,0,'C');
    $this->Cell(10,5,'Hora',0,0,'L');
    $this->Cell(25,5,'Sucursal',0,0,'C');
    $this->Cell(60,5,'Cliente',0,0,'C');
    $this->Cell(6,5,'Edo',0,0,'C');
    $this->Cell(10,5,'T.P.',0,0,'C');
    $this->Cell(19,5,'Importe',0,0,'C');
    $this->Cell(21,5,'A cuenta',0,0,'C');
...
fa.porc_descuento
FROM so_factura fa,kg_tipo_pago tp,so_clientes cl,kg_sucursales su
...
//echo $sql;
      ...
      $pdf->SetFont('Arial','','9');
      $pdf->Cell(60,5,"#"."|"."id_factura"."|"."fecha_factura"."|"."hora"."|"."sucursal"."|"."cliente"."|"."estado"."|"."tipopago"."|"."imp_total"."|"."a_cuenta"."|"."resta"."|"."\n",0,0,'L');
      $pdf->ln(5);
if ($result = mysqli_query($con, $sql)) {
  while($row = $result->fetch_assoc())
    {
      ...
      //$pdf->Cell(2);
      $pdf->SetFont('Arial','','9');       
      $pdf->Cell(60,5,$num."|".$row['id_factura']."|".$row['fecha_factura']."|".$row['hora']."|".$row['sucursal']."|".utf8_decode($row['cliente'])."|".$row['estado']."|".$row['tipopago']."|".'$'.number_format($row['imp_total'],2)."|".'$'.number_format($row['a_cuenta'],2)."|".'$'.number_format($row['resta'],2)."|"."\n",0,0,'L');
