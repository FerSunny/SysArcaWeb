...
$stmt = $con->prepare("SELECT
	...
	while($row = $result->fetch_assoc())
    {
    	  ...
    	  $data[] = array(
					...
					"6" => $estado,
					"7" => $row['imp_total'],
					"8" => "<button type='button' class='btn btn-info btn-sm' onclick=update(".$row['estado_factura'].",". $row['id_factura'].",". $row['numero_factura'].",'".$row['telefono_fijo']."','".$row['anios']."')><span  class='fa fa-pencil fa-1x'></span></button>",
