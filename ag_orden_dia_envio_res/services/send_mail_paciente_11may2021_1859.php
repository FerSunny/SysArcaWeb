...
$stmt = $conexion->prepare("SELECT desc_estudio,fk_id_plantilla FROM km_estudios WHERE id_estudio = ?");
...
if($rows > 0)
{
	$stmt->bind_result($desc_estudio,$fk_id_plantilla);
    $stmt->fetch();
}else
{
	echo 3;
...
*/
	...
	$correoDestino = $e_paciente.';'.'marisol.briceno@laboratoriosarca.mx';
