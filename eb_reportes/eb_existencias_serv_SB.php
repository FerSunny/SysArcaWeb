...
if($row = mysqli_fetch_array($resultado))
{
  ...
  $fk_id_servicio =$row['fk_id_servicio'];
}
...
// Cabecera de página
  function Header()
  {
    ...
    global $sucursal,
      ...
      $this->Cell(3);
      // Título
      $this->Cell(240,5,'ESTUDIOS CLINICOS SAN BUENAVENTURA',0,0,'C');
      $this->Ln(5);
      $this->SetFont('Arial','B',12);
      $this->Cell(142,10,'Existencias',0,0,'R');
      $this->Cell(120,10,'ECSB-F003',0,0,'R');
      $this->Ln(10);
      ...
      $this->Cell(53,10,'GENERADO POR EL SERVICIO: ',0,0,'L');
      $this->SetFont('Arial','',9);
      $this->Cell(20,10,$desc_servicio,0,0,'L');
      ...
      $this->SetFont('Arial','B',7);
	    $this->Cell(17,10,'Id-Cod',0,0,'C');
      $this->Cell(35,10,utf8_decode('Proveedor'),0,0,'C');
      $this->Cell(65,10,utf8_decode('Producto'),0,0,'C');
      $this->Cell(19,10,substr(utf8_decode('Categoria'),0,25),0,0,'C');
      $this->Cell(45,10,utf8_decode('U. medida'),0,0,'C');
      
      $this->Cell(10,10,'Costo',0,0,'C');
      $this->Cell(20,10,'Existencias',0,0,'C');
      //$this->Cell(48,10,'DX',0,0,'C');
      $this->Cell(15,10,'Minimo',0,0,'C');
...
and ac.fk_id_servicio = $fk_id_servicio
order by 3
";
//echo $sql;
$sucursal='';
  ...
  while($row = $result->fetch_assoc())
      {
        ...
        $pdf->SetFont('Courier','',7);
	      ...
	      $pdf->Cell(7,5,$row['fk_id_producto'].'-'.$row['cod_producto'],0,0,'R');
        $pdf->cell(3);
        $pdf->Cell(45,5,utf8_decode($row['razon_social']),0,0,'L');

        $pdf->cell(3);
        $pdf->Cell(55,5,utf8_decode(substr($row['desc_producto'],0,34)),0,0,'L');
        $pdf->Cell(40,5,utf8_decode(substr($row['descripcion'],0,25)),0,0,'L');

        $pdf->Cell(20,5,utf8_decode(strtolower(substr($row['unidad_medida'],0,33))),0,0,'L');
