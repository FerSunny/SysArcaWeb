...
// listar datos en la tabla de comisiones
		var listar = function(){
			...
			var table = $("#dt_ocupaciones").DataTable({
				"destroy":true,
				"sRowSelect": "multi",
				"ajax":{
					"method":"POST",
			...
			obtener_data_editar("#dt_ocupaciones tbody", table);
			obtener_id_eliminar("#dt_ocupaciones tbody", table);
		}
// editamos estado civil
		var obtener_data_editar = function(tbody, table){
			$(tbody).on("click", "button.editar", function(){
				var data = table.row( $(this).parents("tr") ).data();
				var id_ocupacion = $("#frmedit #idocupacion ").val( data.id_ocupacion),
					   desc_ocupacion = $("#frmedit #edit1").val( data.desc_ocupacion),
...
// eliminndo la comision
		var obtener_id_eliminar = function(tbody, table){
			$(tbody).on("click", "button.eliminar", function(){
				var data = table.row( $(this).parents("tr") ).data();
				var id_ocupacion = $("#frmEliminarocupacion #idocupacion").val( data.id_ocupacion);
				 desc_ocupacion =$("#frmEliminarocupacion #ocupacion").val(data.desc_ocupacion);
				opcion = $("#frmEliminarocupacion #opcion").val("eliminar");
				console.log(data);
			});
		}
