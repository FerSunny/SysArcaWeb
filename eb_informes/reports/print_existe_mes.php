...
SELECT 	pr.`cod_producto`,
	...
	pv.`razon_social`,
-- EXISTENCIA INICIAL	
	(ac.existencias - IFNULL((SELECT so.cantidad FROM eb_solicitudes so
WHERE so.`fk_id_producto` = pr.cod_producto
AND YEAR(so.`fecha_registro`) = YEAR(ac.`fecha_actualizacion`)
AND MONTH(so.`fecha_registro`) = MONTH(ac.`fecha_actualizacion`)),0)  ) AS existe_inicial,
  ...
  while($row = $result->fetch_assoc())
    {
      ...
      $pdf->Cell(25,5,$row['desc_producto'],0,0,'L');
	  $pdf->Cell(20);
