...
$sql="SELECT  DISTINCT CONCAT(me.nombre,' ',me.a_paterno,' ',me.a_materno) AS medico,
  su.desc_sucursal AS desc_sucursal,
  me.`horario`,
...
if ($result = mysqli_query($conexion, $sql)) {
  while($row = $result->fetch_assoc())
      {    
        ...
        $pdf->Cell(190,5,'Estudios Clinicos San Buenaventura',0,0,'C');
        $pdf->Ln(5);
        $pdf->cell(3);
        ...
        $pdf->Cell(80,5,'('.$row['id_medico'].') '.utf8_decode($row['medico']),0,0,'L');
        $pdf->Cell(15,5,'Horario:',0,0,'L');
        $pdf->Cell(80,5,$row['horario'],0,0,'L');
        $pdf->Ln(5);
        $pdf->cell(3);
        $pdf->Cell(15,5,'Sucursal:',0,0,'L');
        $pdf->Cell(80,5,$row['desc_sucursal'],0,0,'L');
        $pdf->Ln(5);
        $pdf->cell(3);
        $pdf->Cell(15,5,'Direccion:',0,0,'L');
        $pdf->Cell(80,5,utf8_decode($row['direccion']),0,0,'L');
        $pdf->Cell(15,5,'Colonia:',0,0,'L');
        $pdf->Cell(70,5,utf8_decode($row['colonia']),0,0,'L');
        $pdf->Ln(5);
        $pdf->cell(3);
      ...
      //echo $sql;
        ...
        while($row1 = $result1->fetch_assoc())
            {
              ...
              $pdf->Cell(70,5,utf8_decode($row1['estudio']),0,0,'L');
        ...
        $pdf->MultiCell(190,5,'Dr'.$row['sexo'].' '.$row['medico'].' agradecemos su preferencia y le informamos que ya proximamente, podra darle seguimiento a sus participaciones asi como revisar los resultados de sus pacientes en nuestro sitio web www.estudiosclinicosanbuenaventura.com.mx',0,'L');
        $pdf->SetFont('Arial','',8);
        $pdf->Ln(3);
        $pdf->cell(3);
