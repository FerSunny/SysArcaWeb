   ...
   <form name="AltasModulos" action="controladores/registro_modulos.php" method="post">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h2 style="color:blue;text-align:center" class="modal-title">Nuevo Menu</h2>
          <h5 style="color:blue;text-align:center" class="modal-title">Menus</h5>
        </div>
        <div style="color:#000000;background:#EFFBF5" class="modal-body">
          <table border="0" align="center" BGCOLOR=#F5FBEF style="border-bottom:1px solid #819FF7">
<!-- Id Menu -->
              <tr>
                <td class="renglon_titulo"> Id Menu: </td>
...
<!-- nivel del menu -->
              <tr>
                  <td class="renglon_titulo">Nivel Menu: </td>
                  <td class="renglon_valor">
                      <select name="nivel_menu" >
                         <?php
                            $sql="SELECT * FROM se_menu_nivel WHERE estado = 'A'";
                            $rec=mysqli_query($conexion,$sql);
                            while ($row=mysqli_fetch_array($rec))
                            {
                              echo "<option value='".$row['id_nivel_menu']."' >";
                              echo $row['desc_nivel_menu'];
                              echo "</option>";
                            }
                         ?>
                      </select>
...
<!-- Enlace -->
              <tr>
                  <td class="renglon_titulo">Enlace:</td>
                  <td class="renglon_valor" class="form-control">
                      <input type="text" required name="fn_enlace" id="fi_enlace" maxlength="55" size="55" placeholder="Enlace"/>
                   </td>
              </tr>
 ...
 <!-- Fecha de alta -->
              <tr>
                  ...
                  <td class="renglon_valor" class="form-control">
                        <input type="date" required  name="fn_falta" id="fi_falta" maxlength="20" size="20" placeholder="Fecha  Alta"/>
                  </td> 
              </tr>
      ...
      <div class="modal fade" id="modalEditar" tabindex="-1" role="dialog" aria-labelledby="modalEliminarLabel">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <!-- INICIA EL BODY -->
            <div style="color:#000000;background:#EFFBF5" class="modal-body">
             <input type="hidden" id="idmenu" name="idmenu" value="0">
...
<!-- nivel del menu -->
              <tr>
                  <td class="renglon_titulo">Nivel Menu: </td>
                  <td class="renglon_valor">
                      <select name="nivel_menu" id="nivel_menu">
                         <?php
                            $sql="SELECT * FROM se_menu_nivel WHERE estado = 'A'";
                            $rec=mysqli_query($conexion,$sql);
                            while ($row=mysqli_fetch_array($rec))
...
<!-- Fecha de actualización -->
              <tr>
                  <td class="renglon_titulo">Fecha Actuaizacion: </td>
                  <td class="renglon_valor" class="form-control">
                        <input type="text" required name="fn_factualiza" id="fi_factualiza" readonly="readonly" disabled   maxlength="15" size="20" />
                  </td> 
              </tr>              
...
<div class="modal fade" id="modalEliminar" tabindex="-1" role="dialog" aria-labelledby="modalEliminarLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="modalEliminarLabel">Eliminar Menu</h4>
      </div>
      <div class="modal-body">
        <form id="frmEliminarmodulo" action="controladores/eliminar_modulo.php" method="POST">
          Esta seguro de eliminar el menu? <strong data-name=""></strong>
