...
{
    $sql="SELECT desc_sucrursal FROM kg_sucrsales where id_sucursal = ?";
...
function Footer()
{
    ...
    function Tabla()
    {
       global $nombre,$suc,$conexion,$id_detalle;
        ...
        $ano = date("Y");
            ...
            $this->Cell(30,5,'SUCURSAL:',0);
            $this->Cell(30,5,$suc,0,0,'C');
            $this->ln();
            $this->Cell(30,5,'RESPONSABLE:',0);
            $this->Cell(30,5,$nombre,0,0,'C');
            

    $query = "SELECT pro.cod_producto,pro.id_producto,pro.producto,pro.desc_producto,so.cantidad,so.costo_pza,so.cantidad_real,so.costo_real_pza,so.importe_total FROM eb_solicitudes so
    ...
    $result = $conexion->query($query);
            ...
            $this->SetFont('Arial','B',11);
            $this->Cell(34,9,utf8_decode('Código'),1);
            $this->Cell(36,9,'Producto',1);
            $this->Cell(20,9,'Cantidad',1);
            $this->Cell(16,9,'Precio',1);
            $this->Cell(34,9,'Cantidad recibida',1);
            $this->Cell(29,9,'Precio recibido',1);
            $this->Cell(22,9,'Total',1);
            ...
            while ($row = mysqli_fetch_array($result))
            {
                ...
                $costo_real_pza = $row['costo_real_pza'];
                $total = $row['importe_total'];
                $this->SetFont('Arial','',10);
                $this->Cell(34,7,$cod_producto,1);
                $this->Cell(36,7,$producto,1);
                $this->Cell(20,7,$cantidad,1);
                $this->Cell(16,7,$costo_pza,1);
                $this->Cell(34,7,$cantidad_real,1);
                $this->Cell(29,7,$costo_real_pza,1);
