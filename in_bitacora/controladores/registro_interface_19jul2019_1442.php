...
$sql="	SELECT  
		...
		WHERE DATE(pa.pat_Testing_Date) = '$fecha_nota'
		...
		AND fa.`id_factura` = pa.`Pat_CaseNo`
		AND DATE(fa.`fecha_factura`) = DATE(pa.`Pat_Testing_Date`)
		AND pa.`Pat_Disk` = $pat_disk
		union
		SELECT 	fa.fk_id_empresa,fa.id_factura,es.id_estudio,p1.orden,p1.tipo,
			p1.concepto,p1.codigo_int,p1.valor_refe,p1.unidad_medida,p1.tamfue,
			p1.tipfue,p1.posini,fa.fecha_factura
		FROM km_paquetes pa, km_estudios es,so_detalle_factura df, so_factura fa, cr_plantilla_1 p1,dr_patient pa
		...
		AND fa.`id_factura` $busca_factura
		AND DATE(pa.pat_Testing_Date) = '$fecha_nota'
		AND fa.`id_factura` = pa.`Pat_CaseNo`
		AND DATE(fa.`fecha_factura`) = DATE(pa.`Pat_Testing_Date`)
		AND pa.`Pat_Disk` = $pat_disk
		";
...
//RELA A/G
		...
		if ($valor_regresa > 0.0) {
			$v_valor_ALB = $valor_regresa;
			$valor_regresa=busca_tester($fecha_test, $id_factura ,'GLOB');
			if ($valor_regresa > 0.0) {
				$v_valor_GLOB = $valor_regresa;
				$v_valor = $v_valor_ALB/$v_valor_GLOB;
			}else{
				$resu = insert_log($fk_id_sucursal, $fk_id_empresa,$id_factura,$busca_estudio,'GLOB');
				$v_valor = $valor_regresa;
			}	
		}else{
			$resu = insert_log($fk_id_sucursal, $fk_id_empresa,$id_factura,$busca_estudio,'ALB');
