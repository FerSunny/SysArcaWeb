...
if ($result = mysqli_query($con, $sql_max)) {
  while($row = $result->fetch_assoc())
  {
      $veces=$row['num_imp']+1;
      //echo $veces;
      $sql_update="UPDATE cr_plantilla_1_re SET num_imp = '".$veces."'
      where fk_id_factura=".$numero_factura." and fk_id_estudio=".$studio;
      //echo $sql_update;
...
$sql="(select valor as valor_solo,tipo,orden,concepto,concat(LPAD(trim(valor),15,' '),' ',unidad_medida,verificado) as valor,concat(unidad_medida,verificado) as unidad_medida,verificado,observaciones,CASE
  WHEN fk_id_estudio in (274,963) THEN
        CONCAT(valor_refe) 
  ELSE
...
*/
        if($studio=='274' or $studio =='467' or $studio =='215' or $studio=='963'){
          $justifica='C';
        }else{
          $justifica='R';
        }

        if($row['tipo']=='P' && strlen($row['valor_solo'])==0 and ($studio=='274' or $studio=='151' or $studio=='152' or $studio=='153' or $studio=='904' or $studio=='905' or $studio=='494' or $studio=='279' or $studio=='395' or $studio=='117' or $studio=='525' or $studio=='527' or $studio=='528' or $studio=='526' or $studio=='963' or $studio=='956' or $studio=='957' or $studio=='958' )){
            $nle+=1;
            $nle-=1;
        }else{
             // $this->ln(5)
              ...
              if($row['valor_solo']=='.'){
                ...
                if($studio =='215' && strlen($row['valor_solo'])==0){
                  ...
                  if($studio =='215' && ($row['valor_solo']=='X' or $row['valor_solo']=='x')){
                    $pdf->Cell(26,5,'('.$row['valor_solo'].')',0,0,$justifica);
                  }else{
                    $pdf->Cell(26,5,$row['valor_solo'],0,0,$justifica);
                  } 
                }
