					...
					success: function(data)            

						{
...

							{
								...
								swal('La asociacion estudio-concepto, es erronea')
...
var editar = function(tbody, table) {
...

		{
				...
				$("#frmedit  #dc").val(data.id_concepto)

				$("#frmedit  #pro").val(data.id_concepto)

				$("#frmedit  #codigo").val(data.id_concepto)

				$("#frmedit  #id_estudio").val(data.fk_id_estudio)
				$("#frmedit  #id_plantilla").val(data.fk_id_plantilla)

				$("#frmedit  #estudio").val(data.fk_id_estudio)


				$.post("in_select_estudios.php?val=1", {'id_estudio' : data.fk_id_estudio, 'id_valor' : data.fk_id_valor}  , function(res,status)
      			{
      				console.log(res)
      				$("#frmedit #concepto").html(res);
      				$("#frmedit #concepto option[value='"+data.fk_id_concepto+"']").attr("selected",true);

          		});



				//$("#frmedit  #concepto").val(data.concepto)

				$("#frmedit  #edadi").val(data.edad_inicial)

				$("#frmedit  #edadf").val(data.edad_final)

				$("#frmedit  #genero").val(data.fk_id_sexo)

				$("#frmedit  #rangoi").val(data.rango_inicial)

				$("#frmedit  #rangof").val(data.rango_final)
		...
		$(tbody).on("click", "button.eliminar", function() {
			...
			swalWithBootstrapButtons({
				...
				if (result.value) {
					 ...
					 $.post("./controladores/eliminar.php", {'id_producto' : data.id_concepto}  , function(data,status)
