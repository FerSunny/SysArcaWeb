		...
		var listar = function(){
			...
			var table = $("#dt_productos").DataTable({
				...
				"columns":[
					...
					{"data" : "fk_id_factura"},
					{"data" : "nombre"},
					{"defaultContent":"<button type='button' class='eliminar btn btn-danger btn-sm'><i class='fas fa-trash-alt'></i></button>"}
				],
					...
					success: function(data)            
						{	
							if(data==1)
							{
								...
								swal('Muestra correcta y aceptada')
								console.log(data)
							}else
							if(data == 1062)
							{
							...
							if(data == 2)
							{
								swal('El codigo del producto no existe')
							}else								
							{
...
var eliminar= function(tbody, table) {
		$(tbody).on("click", "button.eliminar", function() {
			...
			swalWithBootstrapButtons({
				...
				if (result.value) {
					 $.post("./controladores/eliminar.php", {'fk_id_toma' : data.fk_id_toma}  , function(data,status)
					{
						swalWithBootstrapButtons(
						'Eliminado!',
