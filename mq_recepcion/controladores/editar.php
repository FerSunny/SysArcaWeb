...
$id_factura_new = $prefolio.substr("000{$ultimo_dia}", -$str_folio);
// fin
    ...
    $queryinsert_fa = "
        INSERT INTO `so_factura`
                    (`fk_id_empresa`,`fk_id_sucursal`,`id_factura`,`numero_factura`,`fecha_factura`,`fk_id_cliente`,`fk_id_medico`,
                     ...
                     `publicidad`,`grupo`,fk_id_factura_maq,fecha_ctl,cose_dia,id_factura_new,id_factura_old
                     )
        VALUES (
        ...
        '".$muestra_fm['grupo']."',
        $id_factura, -- fk_id_factura_maq
        $prefolio, -- fecha_ctl
        $ultimo_dia, -- cose_dia
        $id_factura_new, -- id_factura_new
        0    -- id_factura_old
        )
        ";     
          if(mysqli_query($conexion,$queryinsert_fa))
          {
            $id_factura_new1 = $conexion->insert_id;
            $codigo=1; //echo 1;
          }else{
            $codigo = mysqli_errno($conexion).$queryinsert_fa;
            $id_factura_new1 =0;
          }
  }else{
    $codigo=2;
...
echo "<script>console.log('antes dino " .$result_5. "' );</script>";
            if ($result_5) {
// *** Actualizamos la tabla de maquilas de DINO  ***
echo "<script>console.log('entro dino: " .$id_factura_new. "' );</script>";
              $query_6 = "UPDATE ma_maquila 
                          SET fk_id_folio_maquila =$id_factura_new 
                          WHERE fk_id_factura = $id_factura 
                          AND fk_id_estudio = $id_estudio_ori";
echo "<script>console.log('update dino: " .$query_6. "' );</script>";                          
