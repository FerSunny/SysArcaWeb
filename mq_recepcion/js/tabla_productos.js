			...
			var table = $("#dt_productos").DataTable({
				...
				/*	{"data" : "costo"},
					...
					*/
					...
					{"defaultContent":"<button disabled = 'true' type='button' class='eliminar btn btn-danger btn-md'><i class='fas fa-trash-alt'></i></button>"}
				],
...
var editar = function(tbody, table) {
		$(tbody).on("click", "button.editar", function() 
		{
				...
				$("#frmedit  #dc").val(data.fk_id_estudio_maq)
				$("#frmedit  #pro").val(data.id_factura)
				$("#frmedit  #zapo").val(data.fk_id_estudio)

				$("#frmedit  #id_estudio").val(data.fk_id_estudio_maq)
				$("#frmedit  #id_factura").val(data.id_factura)
				$("#frmedit  #estudio").val(data.desc_estudio)
				$("#frmedit  #caducidad").val(data.fecha_envio)
				$("#frmedit  #observa").val(data.observa)
				$("#frmedit  #costo_maq").val(data.costo_maq)
				/*
				$("#frmedit  #id_estudio_ori").val(data.fk_id_estudio)
...
var eliminar= function(tbody, table) {
		$(tbody).on("click", "button.eliminar", function() {
			...
			swalWithBootstrapButtons({
				...
				if (result.value) {
					 $.post("./controladores/eliminar.php", { 'id_factura' : data.fk_id_factura_maq, 'fk_id_estudio' : data.fk_id_estudio_maq, 'costo_mq' : data.costo_maq }  , function(data,status)
					{
						swalWithBootstrapButtons(
						'Eliminado!',
