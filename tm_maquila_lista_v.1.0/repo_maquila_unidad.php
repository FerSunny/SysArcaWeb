...
// Cabecera de página
  function Header()
  {
    ...
    global $sucursal;
      ...
      $this->Cell(120,10,'ARCA-F001',0,0,'R');
      $this->Ln(10);
      ...
      $this->Cell(35,10,'NOTA',0,0,'C');
      $this->Cell(11,10,'FOLIO',0,0,'C');
      $this->Cell(10,10,'EDAD',0,0,'C');
      $this->Cell(53,10,'PACIENTE',0,0,'C');
      
      $this->Cell(55,10,'ESTUDIO',0,0,'C');
      // $this->Cell(40,10,'MEDICO',0,0,'C');
      //$this->Cell(48,10,'DX',0,0,'C');
      $this->Cell(30,10,'FECHA ENTREGA',0,0,'C');
      $this->Cell(20,10,'EGA',0,0,'C');
      $this->Ln(4);
      $this->Cell(18,8,'DIAGNOSTICO',0,0,'C');
      $this->Ln(1);
...
FROM tm_tomas tm, so_factura fa, so_clientes cl, km_estudios es,

 kg_sucursales su, so_medicos me
...
and es.maquila = 'S'
AND tm.id_toma NOT IN (SELECT sm.fk_id_toma FROM tm_sigue_muestra sm WHERE sm.estado = 'A')

AND DATE(tm.fecha_toma) = CURDATE() 
AND fa.fk_id_medico = me.id_medico
order by tm.fk_id_factura
";
//echo $sql;
$sucursal='';
$num_muestras=0;
  ...
  while($row = $result->fetch_assoc())
      {
          ...
          $pdf->SetFont('Courier','',7);
  	      $pdf->Cell(19,5,$row['lote'],0,0,'L');
          $pdf->Cell(30,5,$row['observaciones'],0,0,'L');
  	      $pdf->SetFont('Courier','',9);
          $pdf->cell(3);
          $pdf->Cell(13,5,$row['fk_id_factura'],0,0,'R');
          $pdf->Cell(5,5,$row['anios'],0,0,'R');
          if ($v_nombre <> $row['nombre'])
          {
            $pdf->Cell(65,5,utf8_decode(strtolower(substr($row['nombre'],0,33))),0,0,'L');
            $v_nombre = $row['nombre'];
          }else
          {
            $pdf->Cell(65,5,' ',0,0,'L');
          ...
          // $pdf->cell(40,5,substr($row['medico'],0,20),0,0,'L');
          //$pdf->Cell(40,5,substr($row['diagnostico'],1,20),0,0,'L');
          $pdf->Cell(35,5,$row['fecha_entrega'],0,0,'L');
          $pdf->Cell(40,5,$row['via'],0,0,'L');
          $pdf->Ln(3);
          $pdf->cell(-1);
          $pdf->SetFont('Arial','',7);
          $pdf->Cell(42,5,$row['diagnostico'].' -- '.$row['datos_clinicos'],0,0,'L');          
