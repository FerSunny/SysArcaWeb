...
WHERE pq.estado = 'A'
  ...
  AND df.fk_id_estudio IN (SELECT DISTINCT fk_id_estudio FROM cr_plantilla_4 WHERE estado = 'A')
   ) res
LEFT OUTER JOIN cr_plantilla_4_re p2 ON (p2.fk_id_factura = res.id_factura AND p2.fk_id_estudio = res.fk_id_estudio)
union
...
LEFT OUTER JOIN cr_plantilla_4_re p2 ON (p2.fk_id_factura = df.id_factura AND p2.fk_id_estudio = df.fk_id_estudio),
km_estudios es
