...
// Pie de página
  function Footer()
  {
    ...
    $sql="SELECT p2.concepto,posini,tipfue,tamfue FROM cr_plantilla_4 p2 WHERE p2.fk_id_estudio =".$studio." AND p2.estado = 'A' AND p2.tipo = 'F' order by orden";
    if ($result = mysqli_query($con, $sql)) {
      while($row = $result->fetch_assoc())
        {
...
$sql="(select valor,tipo,orden,concepto,observaciones,tamfue,posini,tipfue FROM cr_plantilla_4_re
  where fk_id_factura=".$numero_factura." and fk_id_estudio=".$studio.") 
  UNION
      (SELECT null,p2.tipo,p2.orden,p2.concepto,null,tamfue,posini,tipfue FROM cr_plantilla_4 p2
    ...
    while($row = $result->fetch_assoc())
      {
        ...
        //$pdf->Cell(20);
        $pdf->Cell($row['posini']);
        $pdf->SetFont('Arial','',$row['tamfue']);
        $pdf->MultiCell(180,5,utf8_decode($row['valor']),0,'L');
          $pdf->ln(4);
       }
      }
