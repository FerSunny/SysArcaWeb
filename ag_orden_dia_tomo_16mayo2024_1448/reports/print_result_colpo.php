...
$sql_max="select max(num_imp) as num_imp FROM cr_plantilla_colpo_re
...
if ($result = mysqli_query($con, $sql_max)) {
  while($row = $result->fetch_assoc())
  {
      $veces=$row['num_imp']+1;
      //echo $veces;
      $sql_update="UPDATE cr_plantilla_colpo_re SET num_imp = '".$veces."'
      where fk_id_factura=".$numero_factura." and fk_id_estudio=".$studio;
      //echo $sql_update;
...
if ($result = mysqli_query($con, $sql_usg)) {
  while($row = $result->fetch_assoc())
  {
     ...
     //$firma=$row['firma'];
      ...
      $d_comentarios=($row['d_comentarios']);
...
//$pdf-> WriteHTML(0,'<b>'.$d_diagnostico .'</b>');
...
//$pdf-> WriteHTML(0,$d_comentarios);
//$pdf->WriteHTML('<b>You can</b>   <br><p align="center">center a line</p>and add a horizontal rule:<br><hr>');
