...
function Footer()
{
	...
	function Tabla()
	{
	   ...
	   $stmt = $conexion ->prepare("SELECT * FROM se_usuarios WHERE id_usuario=?");
		...
		$this->Ln(15);
		$this->SetFont('Arial','B',10);
		$this->Cell(5,15,'RESPONSABLE: '.$usuario.' '.$a_p.' '.$a_m,0);

		$stmt = $conexion ->prepare("SELECT prs.razon_social FROM eb_solicitudes so
		...
		$this->Ln(1);
		$this->SetFont('Arial','B',10);
		$this->Cell(1,35,'PROVEEDOR: '.$razon_social,0);
